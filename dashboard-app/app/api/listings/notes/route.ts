...
type UpdateListingNotesPayload = {
  source?: string
  listingId?: string
  notes?: string
}
...
export const PATCH = async (request: NextRequest): Promise<NextResponse> => {
  ...
  let payload: UpdateListingNotesPayload

  try {
    payload = (await request.json()) as UpdateListingNotesPayload
  } catch {
    return NextResponse.json(
      {
        ok: false,
  ...
  if (!SUPPORTED_COLLECTIONS.has(source) || !listingId || typeof payload.notes !== "string") {
    return NextResponse.json(
      {
        ok: false,
        error: "A valid source, listingId, and notes string are required.",
      },
      { status: 400 },
    )
  }
